...
def send_activation_notifications(modeladmin, request, queryset):
    """Рассылаем оповещение всем пользователям, кто не выполнил активацию"""
    for rec in queryset:
        if not rec.is_activated:
            send_activation_notification(rec)
    modeladmin.message_user(request, 'Песьма с оповещениями отправлены')
...
class NonactivatedFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if val == 'activated':
            ...
            d = datetime.date.day() - datetime.timedelta(days=3)
            return queryset.filter(is_active=False, is_activated=False, date_joined__date__lt=d)
        elif val == 'week':
            d = datetime.date.day() - datetime.timedelta(weeks=1)
            return queryset.filter(is_active=False, is_activated=False, date_joined__date__lt=d)
