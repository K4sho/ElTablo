...
def detail(request, rubric_pk, pk):
    """Контроллер для деталей объявления"""
...
@login_required
def profile_bb_dd(request):
    """Контроллер, добавляющий объявление"""
    if request.method == 'POST':
        # Передаем еще и словарь со всеми полученными файлами
        form = BbForm(request.POST, request.FILES)
        if form.is_valid():
            bb = form.save()
            # Сохраненную запись мы должны передать конструктору набора форм
...
def profile_bb_change(request, pk):
    """Контроллер для правки объявлений"""
    bb = get_object_or_404(Bb, pk=pk)
    if request.method == 'POST':
